...
class CreditCardLoanBloc extends Bloc<CreditCardLoanEvent, CreditCardLoanState> {
      ...
      if (isBiometricEnabled) {
        ...
        if (isAvailable && isEnrolled) {
          ...
          final Map<String, dynamic> biometricResult = await _biometricService.authenticate(
            reason: 'Authenticate to confirm ${currentState.bankName} payment of \$${currentState.totalAmount.toStringAsFixed(2)}',
          );

          if (!biometricResult['success']) {
            // Biometric authentication failed
            emit(CreditCardLoanError(
          ...
          emit(const CreditCardLoanError(
  ...
  String _buildNotificationMessage(CreditCardLoanDataSet state, bool authenticatedWithBiometric) {
    final DateTime now = DateTime.now();
